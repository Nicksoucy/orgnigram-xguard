...
function schedBuildMonthGrid() {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    headerHTML += `<th style="color:${color};${opacity}${isHoliday ? 'background:rgba(59,130,246,0.07);' : ''}line-height:1.2;" title="${isHoliday ? 'Jour férié' : ''}">
  ...
  rows.forEach(({ trainer, quart, label }) => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (matchEntries.length > 0) {
        ...
        bodyHTML += `<td class="${tdClass}" style="${selStyle};padding:2px;vertical-align:top;${isHoliday ? 'background:rgba(59,130,246,0.05);' : ''}">
          ${stackedHTML}
        </td>`;
      } else {
        const holidayStyle = isHoliday ? 'background:rgba(59,130,246,0.05);' : '';
